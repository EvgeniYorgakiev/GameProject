...
namespace GameConsole
{
    class GameConsole
    {
        ...
        public static List<HeroAttributes> allHeroes = new List<HeroAttributes>()
        {
                ...
                try
                {
                    for (int i = 0; i < heroes.Length; i++) // Saves the heroes
                    {
                        ...
                        foreach (var field in fields)
                        {
                            ...
                            else if (type == typeof(HeroAttributes.Ability)) // We need only the fields that are with a type of item
                            {
                                FieldInfo[] itemFields = type.GetFields(); // Gets all of fields of the current item slot
